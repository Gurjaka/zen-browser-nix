...
stdenv.mkDerivation (finalAttrs: {
  ...
  in
    fetchzip {
      ...
      hash = "sha256-8dyY/Czhnmtw0EoBg0UCGTtlsDqdKW8UM1vcbPHVGlM=";
    };
  ...
  installPhase = ''
    mkdir -p $out/{bin,opt} && cp -r $src/* $out/opt/
    ln -s $out/opt $out/bin/zen

    install -D $desktopSrc/zen.desktop $out/share/applications/zen.desktop

    for i in 16 32 48 64 128; do
        install -D $src/browser/chrome/icons/default/default$i.png \
  ...
  in ''
    chmod 755 $out/bin/* $out/opt/*
    interpreter=$(cat $NIX_CC/nix-support/dynamic-linker)

    for bin in zen zen-bin; do
       patchelf --set-interpreter "$interpreter" $out/opt/$bin
       wrapProgram $out/opt/$bin \
